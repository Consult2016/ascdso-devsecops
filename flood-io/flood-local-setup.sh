...
# flood-local-setup.sh in https://github.com/wilsonmar/DevSecOps/flood-io

# This script installs utilities for using Flood.io Element scripts run in the AWS cloud.
# as described in https://wilsonmar.github.io/flood-perftest.
...
if ! command_exists element-cli ; then  # not exist:
   if [ "$INSTALL_UTILITIES" = "no" ]; then
      ...
      if [ "$PLATFORM" = "macos" ]; then
         if [ "$INSTALL_USING_BREW" = "yes" ]; then
            ...
            brew install flood-io/taps/element
                #   ==> Downloading https://registry.npmjs.org/@flood/element-cli/-/element-cli-1.0.
                # 
         else
            install_element_cli
         fi 
   ...
   if [ "$INSTALL_UTILITIES" = "reinstall" ]; then
      ...
      if [ "$PLATFORM" = "macos" ]; then
         if [ "$INSTALL_USING_BREW" = "yes" ]; then
            brew upgrade flood-io/taps/element
