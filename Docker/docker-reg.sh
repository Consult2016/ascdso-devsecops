...
# Adapted from https://gist.github.com/alexanderilyin/8cf68f85b922a7f1757ae3a74640d48a
...
TOKEN=$(curl -v -X GET -u ${DOCKER_HUB_USER}:${DOCKER_HUB_PASSWORD} \
   "https://${AUTH_DOMAIN}/token?service=${AUTH_SERVICE}&scope=${AUTH_SCOPE}&offline_token=${AUTH_OFFLINE_TOKEN}&client_id=${AUTH_CLIENT_ID}" \
   | jq -r '.token')
VERSIONS=$(curl -v -H "Authorization: Bearer ${TOKEN}" \
   https://${API_DOMAIN}/v2/${DOCKER_HUB_ORG}/${DOCKER_HUB_REPO}/tags/list \
   | jq -r '.tags[]' | grep -E 'v[0-9]+\.[0-9]+\.[0-9]+' | sort --version-sort)
